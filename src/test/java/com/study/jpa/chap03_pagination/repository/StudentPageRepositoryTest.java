...
//                Sort.by("name").descending() // 정렬 기준은 필드명!! 컬럼명(x)
                ...
                // PageRequest's parent(AbstractPageRequest) implements Pageable
                // convey page number and amount of data that will be shown
                // Sort.by(): should convey entity's field name / default = ascending
                // Sort.by(Sort.Order.asc(), ~): order using more than two conditions

        //when
        Page<Student> students = pageRepository.findAll(pageInfo);
                // convey an object which implements Pageable to 'findAll'(provided method)
                // then will get Page object

        // 페이징이 완료된 총 학생 데이터 묶음.
        List<Student> studentList = students.getContent();

        // 총 페이지 수 (get from Page object=students)
    ...
    void testSearchAndPagination() {
        ...
        int totalPages = students.getTotalPages();
        long totalElements = students.getTotalElements();
        boolean next = students.hasNext(); // cannot use this value for displaying buttons
        boolean prev = students.hasPrevious();
        ...
        */
        ...
        //then
        System.out.println("\n\n\n");
        System.out.println("totalPages = " + totalPages); // 보정된 끝 번호
        System.out.println("totalElements = " + totalElements);
        System.out.println("prev = " + prev);
        System.out.println("next = " + next);
