...
class DepartmentRepositoryTest {
    ...
    void testLazyEager() {
        ...
        // select without executing JOIN
        // Lazy executes JOIN if necessary(preferred)
        System.out.println("\n\n\n");
    }
    ...
    void testChangeDept() {
        ...
        Department newDept = departmentRepository.findById(2L)
                .orElseThrow();
        // 2번 -> 사원의 부서 정보를 업데이터 하면서, 부서에 대한 정보도 같이 업데이트
        ...
        // 1번 -> 변경 감지(더티 체크) 후 변경된 내용을 DB엣 즉시 반영하는 역할.
...
//        entityManager.clear(); // 영속성 컨텍스트 비우기(비우지 않으면 컨텍스트 내의 정보를 참조하려 함.)
        ...
        //then
            ...
            // problem: modified data x reflected b/c select is first executed before save()
            // 1) save() -> commit -> inquire(select)
            // 2) put modified data of employee whose id=1 into department whose id=2
    }
    ...
    void testNPlus1Solution() {
        ...
        List<Department> departments
                = departmentRepository.findAllIncludesEmployees(); // JPQL

            // JOIN FETCH -> only one query
