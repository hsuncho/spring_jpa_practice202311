...
@SpringBootTest
@Transactional
@Rollback(false) // for remain data
...
//    @BeforeEach
    ...
    void testFindOne() {
        ...
        //when
        Employee employee = employeeRepository.findById(id) // return type = Optional object
                .orElseThrow(() -> new RuntimeException("사원이 존재하지 않음!"));
            // Optional obj provide method orElseThrow()
            // if present, get employee object but if not, show error message
    ...
    void testFindDept() {
        ...
        // x ambi-directional mapping so cannot get info of employees from department
